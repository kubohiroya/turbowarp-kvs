...
function assertActive(released, signal) {
    if (released)
        throw objectError('KVS_BINARY_STORE_RELEASED', 'Binary object store was released.');
    if (signal?.aborted)
...
export function createOpfsBinaryObjectStore(options = {}) {
    ...
    async function objectName(key) {
        return toHex(new Uint8Array(await subtleCrypto.digest('SHA-256', new TextEncoder().encode(key))));
    }
    ...
    const store = {
        ...
        async cleanupStaging(operationOptions = {}) {
            ...
            for await (const entry of staging.values()) {
                ...
                if (file.lastModified > createdBefore)
                    continue;
        ...
        async getStats(operationOptions = {}) {
            ...
            const pendingDeletionNames = new Set();
            ...
            for await (const prefix of objects.values()) {
                ...
                for await (const entry of prefix.values()) {
                    ...
                    if (pendingDeletionNames.has(entry.name))
                        pendingDeletionBytes += file.size;
...
export function createIndexedDBBinaryObjectStore(options = {}) {
    if (!options || typeof options !== 'object' || Array.isArray(options)) {
    ...
    const open = () => {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onerror = () => reject(objectError('KVS_BINARY_OBJECT_INDEXEDDB_UNAVAILABLE', 'IndexedDB open failed.', request.error));
            request.onblocked = () => reject(objectError('KVS_BINARY_OBJECT_INDEXEDDB_UNAVAILABLE', 'IndexedDB open was blocked.'));
        });
    };
        ...
        async put(descriptorValue, source, operationOptions = {}) {
            ...
            try {
                ...
                if (operationOptions.signal?.aborted)
                    throw abortError(operationOptions.signal.reason);
                if (error instanceof DOMException && error.name === 'QuotaExceededError') {
