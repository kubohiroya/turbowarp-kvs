...
function operationSignal(options: BinaryBundleOperationOptions): AbortSignal | undefined {
  if (!options || typeof options !== 'object' || Array.isArray(options)) {
    throw bundleError(
      'KVS_BINARY_BUNDLE_INPUT_INVALID',
...
async function verifyIntegrity(
  ...
  if (actual !== payload) {
    throw bundleError(
      'KVS_BINARY_BUNDLE_INTEGRITY_MISMATCH',
...
function mappedStoreError(error: unknown, operation: string, signal?: AbortSignal): Error {
  ...
  if (error instanceof DOMException && error.name === 'QuotaExceededError') {
    return bundleError(
      'KVS_BINARY_BUNDLE_QUOTA_EXCEEDED',
      `Binary bundle ${operation} exceeded IndexedDB quota.`,
      error
    );
  }
  return bundleError(
    'KVS_BINARY_BUNDLE_TRANSACTION_FAILED',
...
function createIndexedDBBinaryBundleStore(options: BinaryBundleStoreOptions = {}): BinaryBundleStore {
  ...
  async function openDatabase(): Promise<IDBDatabase> {
    ...
    if (!indexedDB || typeof indexedDB.open !== 'function') {
      throw bundleError(
        'KVS_BINARY_BUNDLE_INDEXEDDB_UNAVAILABLE',
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onblocked = () =>
        rejectOnce(
          bundleError(
            'KVS_BINARY_BUNDLE_INDEXEDDB_BLOCKED',
  ...
  async function normalizeFiles(
    ...
    if (input.files.length > limits.maxFilesPerBundle) {
      throw bundleError(
        'KVS_BINARY_BUNDLE_LIMIT_EXCEEDED',
    ...
    if (!subtleCrypto || typeof subtleCrypto.digest !== 'function') {
      throw bundleError(
        'KVS_BINARY_BUNDLE_CRYPTO_UNAVAILABLE',
  ...
  async function touchBundle(
    ...
    if (!metadata) throw bundleError('KVS_BINARY_BUNDLE_NOT_FOUND', 'Binary bundle was not found.');
    await touch(key.key, metadata.writeToken);
  }
