...
function requireString(value: unknown, label: string, maxLength: number): string {
  if (
    ...
    throw sessionError(
      'KVS_SESSION_BINARY_INPUT_INVALID',
...
function currentTime(now: () => number): number {
  ...
  if (!Number.isSafeInteger(value) || value < 0) {
    throw sessionError(
      'KVS_SESSION_BINARY_CLOCK_INVALID',
...
function ownBytes(value: unknown): OwnedBytes {
  ...
  throw sessionError(
    'KVS_SESSION_BINARY_SOURCE_INVALID',
...
function normalizeInput(
  ...
  if (input.assets.length > limits.maxSessionAssets) {
    throw sessionError(
      'KVS_SESSION_BINARY_LIMIT_EXCEEDED',
...
async function verifyIntegrity(
  ...
  integrity: string,
  subtleCrypto: SubtleCrypto | undefined,
  code: 'KVS_SESSION_BINARY_SOURCE_INTEGRITY_MISMATCH' | 'KVS_SESSION_BINARY_INTEGRITY_MISMATCH'
): Promise<void> {
  if (!subtleCrypto || typeof subtleCrypto.digest !== 'function') {
    throw sessionError(
      'KVS_SESSION_BINARY_CRYPTO_UNAVAILABLE',
  ...
  try {
    ...
    throw sessionError(
      'KVS_SESSION_BINARY_CRYPTO_FAILED',
...
async function readSourceAsset(
  ...
  try {
    ...
    if (errorCode(error).startsWith('KVS_SESSION_BINARY_SOURCE_')) throw error;
    throw sessionError(
      'KVS_SESSION_BINARY_SOURCE_READ_FAILED',
  ...
  for (const loadedFile of loadedRecord.files) {
    ...
    await verifyIntegrity(
      ...
      expected.integrity,
      subtleCrypto,
      'KVS_SESSION_BINARY_SOURCE_INTEGRITY_MISMATCH'
    );
...
function mapStorageError(error: unknown, operation: string, signal?: AbortSignal): Error {
  ...
  if (errorCode(error).startsWith('KVS_SESSION_BINARY_')) return error as Error;
  ...
  if (isDomException(error, 'InvalidStateError')) {
    return sessionError(
      'KVS_SESSION_BINARY_CONNECTION_CLOSED',
      `Session binary ${operation} used a closed IndexedDB connection.`,
      error
    );
  }
  return sessionError(
    'KVS_SESSION_BINARY_TRANSACTION_FAILED',
...
async function addSession(
  ...
  try {
    ...
    if (isDomException(error, 'ConstraintError')) {
      throw sessionError(
        'KVS_SESSION_BINARY_SESSION_CONFLICT',
...
async function readBundle(
  ...
  for (const file of record.files) {
    ...
    await verifyIntegrity(
      ...
      file.integrity,
      subtleCrypto,
      'KVS_SESSION_BINARY_INTEGRITY_MISMATCH'
    );
...
async function releaseSource(source: SessionBinaryBackingSource): Promise<void> {
  try {
    ...
    throw sessionError(
      'KVS_SESSION_BINARY_SOURCE_RELEASE_FAILED',
  ...
  const backing: SessionBinaryBacking = {
    ...
    get(value, operationOptions = {}) {
      const operation = (async () => {
        ...
        try {
          ...
          const normalized =
            error instanceof Error ? error : sessionError('KVS_SESSION_BINARY_READ_FAILED', 'Session binary read failed.', error);
    ...
    dispose() {
      ...
      disposePromise = (async () => {
        ...
        if (errors.length > 0) {
          throw sessionError(
            'KVS_SESSION_BINARY_CLEANUP_FAILED',
